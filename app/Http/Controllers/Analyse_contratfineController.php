...
class Analyse_contratfineController extends Controller
{
  ...
  public function contrat(Request $request)
  {
    ...
    //*********CTI *******************************
        $cti_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                        ...
                        ->where('employes.entite','CTI')->where('statut','ACTIVE')
    ...
    //*********CAC**********************************
                            $cac_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                            ->select('employes.entite','employes.nom','contrats.type_contrat')
                                            ->where('employes.entite','CAC')->where('statut','ACTIVE')
                                            ->where('contrats.type_contrat','CDD')->count();
    ...
    //***********COFINA SENEGAL****************************
                            $cofinasn_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                                  ->select('employes.entite','employes.nom','contrats.type_contrat')
                                                  ->where('employes.entite','COFINA SN')->where('statut','ACTIVE')
                                                  ->where('contrats.type_contrat','CDD')->count();
    ...
    $cofinagn_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                      ->select('employes.entite','employes.nom','contrats.type_contrat')
                                      ->where('employes.entite','COFINA GN')->where('statut','ACTIVE')
                                      ->where('contrats.type_contrat','CDD')->count();
    ...
    $cofinagn_d = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
     ...
     //***********CSG ************************************************
              $csg_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                           ->select('employes.entite','employes.nom','contrats.type_contrat')
                           ->where('employes.entite','CSG')->where('statut','ACTIVE')
                           ->where('contrats.type_contrat','CDD')->count();
    ...
    $cofinacg_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                      ->select('employes.entite','employes.nom','contrats.type_contrat')
                                      ->where('employes.entite','COFINA CG')->where('statut','ACTIVE')
                                      ->where('contrats.type_contrat','CDD')->count();
    ...
    //****************COFINA MALI****************************************
                                       $cofinaml_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                                                         ->select('employes.entite','employes.nom','contrats.type_contrat')
                                                                         ->where('employes.entite','COFINA ML')->where('statut','ACTIVE')
                                                                         ->where('contrats.type_contrat','CDD')->count();
    ...
    //****************FINELLE****************************************
                                       $finelle_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                                                         ->select('employes.entite','employes.nom','contrats.type_contrat')
                                                                         ->where('employes.entite','FINELLE')->where('statut','ACTIVE')
                                                                         ->where('contrats.type_contrat','CDD')->count();
    ...
    ->select('employes.entite','employes.nom','contrats.type_contrat')
    ->where('employes.entite','COFINA BF')->where('statut','ACTIVE')
    ->where('contrats.type_contrat','CDD')->count();
    ...
    ->select('employes.entite','employes.nom','contrats.type_contrat')
    ->where('employes.entite','CPS SN')->where('statut','ACTIVE')
    ->where('contrats.type_contrat','CDD')->count();
    ...
    $cpsml_a = DB::table('employes')->join('contrats','employes.id','=','contrats.employe_id')
                                  ->select('employes.entite','employes.nom','contrats.type_contrat')
                                  ->where('employes.entite','CPS ML')->where('statut','ACTIVE')
                                  ->where('contrats.type_contrat','CDD')->count();
    ...
    ->select('employes.entite','employes.nom','contrats.type_contrat')
    ->where('employes.entite','CPS CI')->where('statut','ACTIVE')
    ->where('contrats.type_contrat','CDD')->count();
    ...
    ->select('employes.entite','employes.nom','contrats.type_contrat')
    ->where('employes.entite','COFINA SERVICES FRANCE')->where('statut','ACTIVE')
    ->where('contrats.type_contrat','CDD')->count();
    ...
    //**********************************************************************
          $cdd = DB::table('employes')->select('employes.id','employes.nom','contrats.type_contrat')->join('contrats','employes.id','=','contrats.employe_id')->where(['statut'=>'ACTIVE'])->where('contrats.type_contrat','CDD')->count();
          $cdi = DB::table('employes')->select('employes.id','employes.nom','contrats.type_contrat')->join('contrats','employes.id','=','contrats.employe_id')->where(['statut'=>'ACTIVE'])->where('contrats.type_contrat','CDI')->count();
          $stage = DB::table('employes')->select('employes.id','employes.nom','contrats.type_contrat')->join('contrats','employes.id','=','contrats.employe_id')->where(['statut'=>'ACTIVE'])->where('contrats.type_contrat','STAGE')->count();
          $prestation = DB::table('employes')->select('employes.id','employes.nom','contrats.type_contrat')->join('contrats','employes.id','=','contrats.employe_id')->where(['statut'=>'ACTIVE'])->where('type_contrat','PRESTATION')->count();
          ...
          $dodo = Charts::create('pie', 'highcharts')
              ...
              //*****************************************************************
              $usr = DB::table('employes')->select('*')->join('contrats','employes.id','=','contrats.employe_id')->where(['employes.statut'=>'ACTIVE'])->get();
              $contratmois = Charts::database($usr, 'bar', 'fusioncharts')
          ...
          //********************************************************************
                                  ...
                                  $chart = Charts::database($usr, 'donut', 'highcharts')

                          			      ->title("Employes")

                                      //->colors(['#800080', '#FF00FF', '#008080','#008000','#800000','#808080','#808000','#CD5C5C'])
                                      ->colors(['#800080', '#008080','#008000','#808080','#808000','#00FF00', '#25FDE9', '#8EA2C6','#FF0000','#791CF8','#2C75FF','#009999','#000080','#FF00FF','#800000', '#CD5C5C'])
        ...
        return view('/finelle/analyse/contrat',['dodo' => $dodo,'contratmois' => $contratmois,
