...
class ComptebfController extends Controller
{
  ...
  //  flash("Merci d'avoir visité votre BI!")->success();
    ...
    public function sessionuser(Request $request)
    {
      ...
      return view('/includes/headerdesktop-maker-bf',['users' => $user]);
    }
 ...
 if(!empty($choisir_entite)){
   //$employes=DB::table('employes')->where('entite', '=',choisir_entite)->get();
    ...
    flash("Vous êtes la bienvenue!!")->success();
     return view('/cofinabf/accueil',['sites' => $sites,'employes' => $employes,'geo' => $geo,'nb_empl' => $nb_empl,'nb_cdi' => $nb_cdi,'nb_cdd' => $nb_cdd,
